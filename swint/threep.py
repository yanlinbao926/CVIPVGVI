...
print("🔥 [核心训练] 当前模式: Swin-T 三视角融合 + Adam + 批次16 + HuberLoss免疫异常值")
CONFIG = {
...
class FlexibleSwinMultiViewModel(nn.Module):
    def __init__(self, bottleneck_dim=768, freeze_backbone=False):
        super(FlexibleSwinMultiViewModel, self).__init__()
        ...
        self.bottleneck = nn.Sequential(
            nn.Linear(768 * 3, bottleneck_dim),  
    ...
    def forward(self, x_f, x_m, x_b):
        ...
        if len(feat_f.shape) > 2:
            feat_f = torch.flatten(feat_f, 1) 
        if len(feat_m.shape) > 2:
...
def run_training():
    ...
    model = FlexibleSwinMultiViewModel(
