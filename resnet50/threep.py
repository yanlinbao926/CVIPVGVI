...
print("🔥 [核心训练] 当前模式: 三视角融合 + Adam + 批次16 + HuberLoss免疫异常值")
CONFIG = {
...
class MultiViewDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_f = Image.open(row['front_path']).convert('RGB')
        img_m = Image.open(row['mid_path']).convert('RGB')
        img_b = Image.open(row['back_path']).convert('RGB')
        ...
        label = torch.tensor(float(row['pedestrian_gvi']), dtype=torch.float32)
        return img_f, img_m, img_b, label


class FlexibleMultiViewModel(nn.Module):
    def __init__(self, bottleneck_dim=2048, freeze_backbone=False):
        super(FlexibleMultiViewModel, self).__init__()
        
        logging.info("🔄 正在从 PyTorch 官方获取 ResNet-50 预训练权重...")
        ...
        logging.info("🔒 Backbone 已冻结" if freeze_backbone else "🔓 Backbone 已解冻")
            
        self.bottleneck = nn.Sequential(
            nn.Linear(2048 * 3, bottleneck_dim),  
    ...
    def forward(self, x_f, x_m, x_b):
        feat_f = self.backbone(x_f)  
        feat_m = self.backbone(x_m)
        feat_b = self.backbone(x_b)
        
        feat_f = torch.flatten(feat_f, 1) 
        feat_m = torch.flatten(feat_m, 1)
        feat_b = torch.flatten(feat_b, 1)
        
        feat_concat = torch.cat([feat_f, feat_m, feat_b], dim=1) 
        
        feat = self.bottleneck(feat_concat)
...
def run_training():
    ...
    train_loader = DataLoader(
        MultiViewDataset(train_df, train_transform), 
    ...
    val_loader = DataLoader(
        MultiViewDataset(val_df, val_transform), 
        batch_size=CONFIG["batch_size"], shuffle=False, 
        num_workers=CONFIG["num_workers"], drop_last=False
    )
    
    test_loader = DataLoader(
        MultiViewDataset(test_df, val_transform), 
        batch_size=CONFIG["batch_size"], shuffle=False, 
        num_workers=CONFIG["num_workers"], drop_last=False
    )
    
    model = FlexibleMultiViewModel(
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        train_loss = 0
        for x_f, x_m, x_b, y in train_loader:
            x_f, x_m, x_b = x_f.to(CONFIG["device"]), x_m.to(CONFIG["device"]), x_b.to(CONFIG["device"])
        ...
        preds, trues = [],[]
        with torch.no_grad():
            for x_f, x_m, x_b, y in val_loader:
                x_f, x_m, x_b = x_f.to(CONFIG["device"]), x_m.to(CONFIG["device"]), x_b.to(CONFIG["device"])
                y = y.to(CONFIG["device"]).unsqueeze(1)
    ...
    with torch.no_grad():
        for x_f, x_m, x_b, y in test_loader: 
            x_f, x_m, x_b = x_f.to(CONFIG["device"]), x_m.to(CONFIG["device"]), x_b.to(CONFIG["device"])
            y = y.to(CONFIG["device"]).unsqueeze(1)
